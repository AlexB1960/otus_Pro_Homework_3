   ...
   node("ansible") {
     ...
     stage("Build Docker image") {
         docker.withRegistry("http://localhost:5005") { //в Registry, который прилагается к дженкинсу
             docker.build("root/api_tests:${env.BRANCH}").push() //собираем докер-образ с именем(имя проекта:имя ветки) и пушим его в Registry для дальнейшего запуска
     ...
     stage("Running api-tests") {
        ansiblePlaybook playbook: "playbook.yml", //плейбука, которая запускает тесты (и разворачивает инфраструктуру)
                extraVars: [
                        branch : "${env.BRANCH}", //передаем в плейбуку ветку, из которой запускаем
